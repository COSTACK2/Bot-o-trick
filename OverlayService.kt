...
/**
 * Serviço em primeiro plano (foreground service) responsável por desenhar
 * e gerenciar a barra flutuante (overlay) que continua funcionando mesmo
...
class OverlayService : Service() {
    ...
    override fun onCreate() {
        ...
        // Garante um valor padrão salvo para "DPI normal" (o DPI original do aparelho)
    ...
    private fun configurarBarraCompleta(view: View) {
        ...
        // Botão "Trick": alterna entre o DPI alto e o DPI normal configurados
        ...
        // Minimiza a barra: ela vira o pequeno botão "Trick" arrastável
        btnMinimizar.setOnClickListener {
            fullBar.visibility = View.GONE
            minimizedButton.visibility = View.VISIBLE
        }

        // Fecha tudo. Se o Trick estiver ativo, restaura o DPI normal antes de sair.
    ...
    /**
     * Uma janela overlay normalmente usa FLAG_NOT_FOCUSABLE (para não roubar
     * o foco/teclado do app por baixo). Isso impede digitar nos EditText.
     * Aqui alternamos essa flag dinamicamente: enquanto um campo de DPI está
     * com foco, a janela vira focável (teclado aparece); ao perder o foco,
     * volta a ser não-focável.
     */
    ...
    private fun avisarFalha() {
        Toast.makeText(
            this,
            "Não foi possível alterar o DPI. É necessário root ou a permissão " +
                "WRITE_SECURE_SETTINGS concedida via ADB (veja o README).",
            Toast.LENGTH_LONG
        ).show()
    }

    /** Permite arrastar a view pela tela, atualizando os parâmetros do WindowManager. */
